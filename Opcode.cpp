...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[SRA].name = "sra"; myArray[SRA].numOps = 3; myArray[SRA].rdPos = 0; myArray[SRA].rsPos = -1; myArray[SRA].rtPos = 1; myArray[SRA].immPos = 2; myArray[SRA].instType = ITYPE; myArray[SRA].op_field= "000000"; myArray[SRA].funct_field = "000011"; //use immediate
...
{
  for(int i = 0; i < (int)UNDEFINED; i++){ //how does this happen?
